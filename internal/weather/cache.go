...
func (cache FileCache) Load() (Snapshot, error) {
	contents, err := os.ReadFile(cache.path)
	if err != nil {
		return Snapshot{}, fmt.Errorf("read cache: %w", err)
	}
	...
	if err := decoder.Decode(&snapshot); err != nil {
		return Snapshot{}, fmt.Errorf("decode cache: %w", err)
	}
	return snapshot, nil
}

func (cache FileCache) Save(snapshot Snapshot) error {
	...
	temporary, err := os.CreateTemp(directory, ".weather-*.json")
	if err != nil {
