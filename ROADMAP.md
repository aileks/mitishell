...
- [ ] v0.7.0: bar, clock, and weather depth. Keyboard-layout and available-update bar widgets. The update widget counts `pacman` and AUR updates without installing anything. Week numbers in the calendar grid, click-cycled clock formats, and timezone selection. Bar islands reorder by dragging and persist through configuration. The tray expands inline and the media island fits its content. Weather location drops GeoClue. The provider detects location from the network by default. A manual location, searchable by place name, overrides detection. Set it from the CLI, the weather popover, or the settings window
