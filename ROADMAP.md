...
- [ ] v0.8.0: new views. Search for emoji in a picker. Show night light state and let users switch an outside tool at run time. Don't add a daemon
- [ ] v0.9.0: the last visual audit before v1.0. Check Web Content Accessibility Guidelines (WCAG) level AA contrast. Body text needs 4.5:1. Large text and UI edges need 3:1. Match space and type with `Theme.qml`. Give each accent one clear role and follow its set meaning. Review each view and state with `make run`. Repeat the pass until the shell feels whole
